...
Greedy : Further building you can reach with k ladders and m bricks.By using ladder u can go to any height.1st push for(i=0 to k) push all elements in pq and assume all climed by ladder then insert for(i=k to n) remove all min element so that they are climed by bricks are limited. If bricks are over this is the building where we can reach.
