...
int countSquares(vector<vector<int>>& matrix) {
		...
		for(int i=0;i<matrix.size();++i)
			    ...
			    matrix[i][j] = 1 + min(matrix[i-1][j],min(matrix[i][j-1],matrix[i-1][j-1]));  //If this index is 1 then max length of square matrix formed starting from this index (in part of matrix we have traversed i,e upward left part considering cur index to centre of graph paper) 
